...
namespace RMC::rBitrage 
{
    ...
    public:
        ...
        void OnFrameUpdate(float deltaTime) 
        {
            ...
            if (velocityCopy.x < 0 && _actor.GetTransform().Position.x - _actor.GetSize().x/2 <= minX)
            {
                velocityCopy.x *= -1;
                hasChangedDirection = true;
            }
                
            else if (velocityCopy.x > 0 && _actor.GetTransform().Position.x + _actor.GetSize().x/2 >= maxX)
            {
                velocityCopy.x *= -1;
                hasChangedDirection = true;
            }
            
            if (velocityCopy.y < 0 && _actor.GetTransform().Position.y - _actor.GetSize().y/2 <= minY)
            {
                velocityCopy.y *= -1;
                hasChangedDirection = true;
            }
                
            else if (velocityCopy.y > 0 && _actor.GetTransform().Position.y + _actor.GetSize().y/2 >= maxY)
            {
                velocityCopy.y *= -1;
                hasChangedDirection = true;
            }

            if (velocityCopy.z < 0 && _actor.GetTransform().Position.z - _actor.GetSize().z/2 <= minY)
            {
                velocityCopy.z *= -1;
                hasChangedDirection = true;
            }
                
            else if (velocityCopy.z > 0 && _actor.GetTransform().Position.z + _actor.GetSize().z/2 >= maxY)
            {
                velocityCopy.z *= -1;
                hasChangedDirection = true;
            }
